...
public class QuestAdapterTests
{
    ...
    public async Task Fallout4QuestAdapter_SortsScriptsByName(
    ...
    [Theory, MutagenModAutoData(GameRelease.Fallout4)]
    public async Task Fallout4QuestAdapter_SortsFragmentsByStage(
        IFileSystem fileSystem,
        DirectoryPath tempDir,
        Fallout4EntryPoint entryPoint,
        Fallout4Mod mod)
    {
        ...
        quest.VirtualMachineAdapter.Fragments.Add(new QuestScriptFragment() { Stage = 400 });
        quest.VirtualMachineAdapter.Fragments.Add(new QuestScriptFragment() { Stage = 5 });
        quest.VirtualMachineAdapter.Fragments.Add(new QuestScriptFragment() { Stage = 200 });
        quest.VirtualMachineAdapter.Fragments.Add(new QuestScriptFragment() { Stage = 75 });
        ...
        initialStages.ShouldBe(new ushort[] { 400, 5, 200, 75, 150 });
        ...
        sortedStages.ShouldBe(new ushort[] { 5, 75, 150, 200, 400 });
    }

    [Theory, MutagenModAutoData(GameRelease.Fallout4)]
    public async Task Fallout4QuestAdapter_SortsBothScriptsAndFragments(
        IFileSystem fileSystem,
        DirectoryPath tempDir,
        Fallout4EntryPoint entryPoint,
        Fallout4Mod mod)
    {
        ...
        // Add scripts in unsorted order
        quest.VirtualMachineAdapter.Scripts.Add(new ScriptEntry() { Name = "Charlie" });
        ...
        // Verify initial order is NOT sorted
        var initialScriptNames = quest.VirtualMachineAdapter.Scripts.Select(s => s.Name).ToArray();
        initialScriptNames.ShouldBe(new[] { "Charlie", "Alpha" });
        var initialStages = quest.VirtualMachineAdapter.Fragments.Select(f => f.Stage).ToArray();
        initialStages.ShouldBe(new ushort[] { 100, 10, 50 });
        ...
        var sortedScriptNames = deserializedQuest.VirtualMachineAdapter?.Scripts.Select(s => s.Name).ToArray();
        sortedScriptNames.ShouldBe(new[] { "Alpha", "Charlie" });

        var sortedStages = deserializedQuest.VirtualMachineAdapter?.Fragments.Select(f => f.Stage).ToArray();
        sortedStages.ShouldBe(new ushort[] { 10, 50, 100 });
    }
}